...
**25 Tem 2026 — İŞ YATIRIM FİNANSAL + LOGO TAMAMLANDI (prod doğrulamalı).** Prod'da (temiz tarayıcı sekmesi, parakonusur.com) SARAE /hisse: **fiyat + F/K 70.80 + PD/DD 7.14 + Özet Finansallar (ekran-görüntüsü-3 formatı)** hepsi ✓ görsel doğrulandı. halka-arz cron `success` (hata:0) tüm 5 arzın logosunu og:image'den doldurdu (ALBTN/KARCL/MASFN/METEN/SARAE). Logo son iyileştirme (`870f2c4`): uzak-host logoUrl artık next/image proxy'siyle SAME-ORIGIN servis ediliyor (Vercel cache + client cross-origin quirk'i bypass) — proxy 200 image/png doğrulandı. **Not:** bu oturumun önizleme tarayıcısı hiçbir `<img>`'i render etmedi (same-origin proxied dahil hepsi 0x0 — sandbox kısıtı), o yüzden logonun GÖRSEL teyidi gerçek tarayıcıda (Chrome/Safari) yapılmalı; sunucu tarafı kanıt (proxy image/png döndürüyor) sağlam. Kalan bilinçli sınır: bankalarda İş Yatırım XI_29 null → TradingView fallback (regresyon yok); F/K 70.80 vs Midas 67 farkı piyasa-değeri/pay-adedi metodolojisi (kabul edilebilir, halkaarz.info 156'dan çok daha doğru). Kod işi YOK.
