...
type TooltipPayloadItem = { value?: number | string; payload?: GrafikPoint };
...
export default function HisseGrafik({ grafik, grafikRange, grafikDegisim, gunlukDusuk, gunlukYuksek, sonFiyat: sonFiyatProp, setGrafikRange, fetchGrafik }: Props) {
  ...
  const degisimEtiketi = grafikRange === "1d" ? "Açılışa göre" : "Başlangıca göre";
  const chartDirection = grafikDegisim === null || Math.abs(grafikDegisim) < 0.005
    ? "flat"
    : grafikDegisim < 0
      ? "down"
      : "up";
      ...
      <div className="hisse-chart-shell">
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={grafik}>
            ...
            <Tooltip content={<ChartTooltip acilisFiyati={acilisFiyati} degisimEtiketi={degisimEtiketi} chartColor={chartColor} />} />
            <Area
              type="monotone"
              dataKey="fiyat"
              stroke={chartColor}
              ...
              dot={(props) => <LastPriceDot {...props} totalPoints={grafik.length} chartColor={chartColor} />}
